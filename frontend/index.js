...
const NOTE_PHOTOS = [
  "WhatsApp Image 2026-04-21 at 4.50.57 PM.jpeg",
  "WhatsApp Image 2026-04-21 at 4.50.56 PM.jpeg",
  "WhatsApp Image 2026-04-21 at 4.54.59 PM (1).jpeg",
  "WhatsApp Image 2026-04-21 at 4.50.56 PM (2).jpeg",
  "WhatsApp Image 2026-04-21 at 4.50.57 PM (1).jpeg",
  "WhatsApp Image 2026-04-21 at 4.50.56 PM (1).jpeg",
  "WhatsApp Image 2026-04-21 at 4.50.57 PM (2).jpeg",
];
function randomPhoto() {
  return NOTE_PHOTOS[Math.floor(Math.random() * NOTE_PHOTOS.length)];
}
...
async function loadNotesFromBackend() {
  try {
    ...
    notes.forEach(note => {
      ...
      card.innerHTML = `
                <img class="msg-thumb" src="${randomPhoto()}" alt="Friend">
                <div class="msg-from">${note.from || 'A Friend'} 💌</div>
...
async function addWish() {
  ...
  try {
    await fetch(`${BACKEND}/note`, {
      ...
      body: JSON.stringify({ from, title, message: body })
    });
  } catch (e) {
    console.log("Backend offline, showing locally anyway");
  }

  // Show card instantly with random image
  const grid = document.querySelector('.messages-grid');
  const card = document.createElement('div');
  card.className = 'message-card dynamic-card';
  card.innerHTML = `
        <img class="msg-thumb" src="${randomPhoto()}" alt="Friend">
        <div class="msg-from">${from || 'A Friend'} 💌</div>
